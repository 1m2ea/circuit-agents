"""
circuit-agents · trace_renderer（观察窗 B · 渲染器 · 仓库内唯一源码）
===============================================================
把执行器的「事件流 + 拓扑」渲染成自包含 HTML（离线可用，无外部依赖）：
  · 拓扑图：节点按最终状态四色着色（绿✓完成 / 红✗失败 / 橙虚框=补数闭环 / 紫⚠=进化）
  · 时间线面板：每个事件一行（相对时间戳 + 类型色点 + 详情）
  · 走查动画：播放/暂停 + 倍速 + 进度条，按发生顺序高亮节点（蓝激活 / 紫脉冲）

复用 circuit-planner 的四色复盘约定（绿=完成/自愈 / 红=失败 / 橙虚=重试 / 紫⚠=进化）。

⚠ 关键：模板用 string.Template（$ 占位），不要用 .format()——CSS/JS 的花括号会被当占位符炸掉。

CANONICAL：本文件是「agent-event-trace」WorkBuddy 技能模式的唯一源码。
技能目录 ~/.workbuddy/skills/agent-event-trace/reference/trace_renderer.py 是注明出处的可复制快照，
供没有本仓库的其他项目直接 vendor 使用。

期望执行器暴露的接口（属性名可改，改完同步本文件即可）：
  executor.circuit.layers()            -> 分层节点 id 列表（每层一个 list）
  executor.circuit.components[cid]     -> {"label", "type", ...}
  executor.circuit.forward             -> [[a, b], ...] 连线
  executor.circuit.spec.get("name")
  executor._events                     -> [{"t","type","scope",...}]
  executor._results                    -> {cid: <obj with .ok, .quality>}
  executor._filled_nodes               -> set(cid)  橙虚框
  executor._evolved_from_node          -> str|None   紫⚠
  executor.state.get("_evolved")       -> dict|None
"""
...
def _node_status(executor, cid):
    ...
    if sig is None:
        return "neutral", None
    return ("ok", sig) if sig.ok else ("fail", sig)
...
def render_executor_trace(executor, title=None, out_path=None):
    """渲染执行追踪产物 HTML。返回写出路径；out_path 缺省为 ./executor_trace.html。

    调用方（如 compiler/demo.py）通常显式传 out_path，例如
    examples/executor_trace.html 与 examples/executor_trace_fill.html。
    """
    ...
    evolved_from = getattr(executor, "_evolved_from_node", None)
    ...
    nodes_svg = []
    filled = getattr(executor, "_filled_nodes", set())
    for cid, (li, yi) in pos.items():
        ...
        if status == "ok":
            mark, qtxt = "✓", f"ok · q={sig.quality:.2f}"
        elif status == "fail":
            mark, qtxt = "✗", "失败"
        else:
            mark, qtxt = "·", "—"
    ...
    total_cost = round(sum(getattr(s, "cost", 0.0) for s in executor._results.values()), 4)
    total_lat = round(max((getattr(s, "latency_ms", 0.0)
                           for s in executor._results.values()), default=0.0), 1)
    evolved = executor.state.get("_evolved") if hasattr(executor, "state") else None
    summary = (f'<span class="k">总耗时</span> {max_t:.0f}ms &nbsp;·&nbsp; '
               f'<span class="k">总成本</span> ${total_cost:.4f} &nbsp;·&nbsp; '
               f'<span class="k">末级延迟</span> {total_lat}ms &nbsp;·&nbsp; '
               f'<span class="k">进化</span> '
               f'{"是 (" + _esc(evolved["spec_name"]) + ")" if evolved else "否"}')

    out_path = out_path or os.path.join(os.getcwd(), "executor_trace.html")
    out_path = os.path.abspath(out_path)
    d = os.path.dirname(out_path)
    if d:                                  # Windows 上纯文件名的 dirname 是 ''，跳过建目录
...
# ---- HTML 模板（自包含、离线可用；用 $ 占位以兼容 CSS/JS 花括号）----
...
<div class="wrap">
  <div class="card topo-card">
    ...
    <div class="legend">
      ...
      <span><i style="background:#eef2f5;border:2.5px solid var(--purple)"></i>触发进化 ⚠</span>
    </div>
  </div>
